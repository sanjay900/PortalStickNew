...
package net.tangentmc.portalStick.util.math;
...
public class Quaternion implements Serializable {
	...
	public Quaternion setFromCross (final Vector3 v1, final Vector3 v2) {
		...
		Quaternion q = setFromAxisRad(v1.y * v2.z - v1.z * v2.y, v1.z * v2.x - v1.x * v2.z, v1.x * v2.y - v1.y * v2.x, angle);
		if (dot == -1) {
			if (!v1.isOnLine(new Vector3(1, 0, 0)))
			q.mulLeft(1, 0, 0, 0);
			else if (!v1.isOnLine(new Vector3(0, 1, 0)))
				q.mulLeft(0, 1, 0, 0);
			else if (!v1.isOnLine(new Vector3(0, 0, 1)))
				q.mulLeft(0, 0, 0, 1);
		}
		dot = MathUtils.clamp(v1.dot(v2), -1f, 1f);
		return q;
	}
